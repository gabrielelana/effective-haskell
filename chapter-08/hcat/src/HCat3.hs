...
showPages :: ScreenDimensions -> [T.Text] -> IO ()
showPages _ [] = return ()
showPages ScreenDimensions{..} (page:pages) = go page [] pages
  where go page prevPages nextPages = do
        ...
        showPage page = do
          clearScreen
          showHelp $ T.justifyLeft screenColumns ' ' "[n]: next page, [p]: previous page, [q]: quit"
          TIO.putStr page
...
runHCat = handle handleError do
  ...
  sd <- considerHelpLine <$> terminalSize
  showPages sd $ paginate sd contents
